...
SEP = "=" * 72
print(SEP)
print("  TRACK B — LSTM  (18 cities × 3 horizons)")
...
for pq in parquets:
    ...
    for h in HORIZONS:
        ...
        ys_te = sc_y.transform(yte_aligned.reshape(-1,1)).ravel()
        ...
        results.append({"city": city, "model": "LSTM", "horizon": h,
...
if CKPT.exists():
    ...
    track_b_lstm_df = pd.concat([old, track_b_lstm_df], ignore_index=True)
sub = track_b_lstm_df.groupby("horizon")["R2"].mean()
track_b_lstm_df.to_csv(CKPT, index=False)

print(f"\n  Saved: {CKPT}  ({len(track_b_lstm_df)} rows)")
print(f"  Track B LSTM: " + " | ".join(f"h{h}h R²={sub.get(h,0):.4f}" for h in HORIZONS))
