...
SEP = "=" * 72
print(SEP)
print("  TRACK A — RANDOM FOREST — All Cities")
print(SEP)
...
for pq in parquets:
    ...
    rmsev = rmse(y_te, yp)
    results.append({"city": city, "model": "RandomForest",
...
track_a_rf_df = pd.DataFrame(results)
if CKPT.exists():
    old = pd.read_csv(CKPT)
    track_a_rf_df = pd.concat([old, track_a_rf_df], ignore_index=True)
track_a_rf_df.to_csv(CKPT, index=False)
print(f"\n  Saved: {CKPT}  ({len(track_a_rf_df)} rows)")
