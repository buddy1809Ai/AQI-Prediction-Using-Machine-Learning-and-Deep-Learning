...
print(SEP)
print("  TRACK A — CNN-BiLSTM — All Cities (18 cities)")
print(SEP)
...
for pq in parquets:
    ...
    model.fit(Xs_tr, ys_tr, validation_data=(Xs_va, ys_va),
              epochs=50, batch_size=128, callbacks=cb_list_base, verbose=0)
    tr_t = time.time() - t0
    t1   = time.time()
    ...
    results.append({"city": city, "model": "CNN-BiLSTM",
                    "n_rows": n, "n_features": c,
                    "R2": round(r2v,4), "MAE": round(maev,2),
                    "RMSE": round(rmsev,2), "train_s": round(tr_t,1)})
    print(f"  [{city}]  n={n:,}  feats={c}  "
          f"R²={r2v:.4f}  MAE={maev:.1f}  RMSE={rmsev:.1f}  t={tr_t:.0f}s")

track_a_cnn_bilstm_df = pd.DataFrame(results)
if CKPT.exists():
    old = pd.read_csv(CKPT)
    track_a_cnn_bilstm_df = pd.concat([old, track_a_cnn_bilstm_df], ignore_index=True)

valid_r2 = track_a_cnn_bilstm_df[track_a_cnn_bilstm_df["R2"].notna()]
track_a_cnn_bilstm_df.to_csv(CKPT, index=False)

print(f"\n{SEP}")
print(f"  Saved: {CKPT}  ({len(track_a_cnn_bilstm_df)} rows)")
if not valid_r2.empty:
    print(f"  Avg R²: {valid_r2['R2'].mean():.4f}  |  "
          f"Best: {valid_r2.loc[valid_r2['R2'].idxmax(),'city']} ({valid_r2['R2'].max():.4f})")
