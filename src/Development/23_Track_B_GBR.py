...
SEP = "=" * 72
print(SEP)
print("  TRACK B — GRADIENT BOOSTING  (18 cities × 3 horizons)  ← BEST FORECASTER")
...
for pq in parquets:
    ...
    for h in HORIZONS:
        ...
        if len(X_te) < 200:
            continue
        t0  = time.time()
        mdl = GradientBoostingRegressor(n_estimators=300, max_depth=5, random_state=SEED)
        mdl.fit(X_tr, y_tr)
        tr_t = time.time() - t0
        yp   = mdl.predict(X_te)
        ...
        rv   = rmse(y_te, yp)
        results.append({"city": city, "model": "GradBoost", "horizon": h,
...
track_b_gbr_df = pd.DataFrame(results)
if CKPT.exists():
    old = pd.read_csv(CKPT)
    track_b_gbr_df = pd.concat([old, track_b_gbr_df], ignore_index=True)

sub  = track_b_gbr_df.groupby("horizon")["R2"].mean()
track_b_gbr_df.to_csv(CKPT, index=False)

print(f"\n  Saved: {CKPT}  ({len(track_b_gbr_df)} rows)")
print(f"  Track B GBR: " + " | ".join(f"h{h}h R²={sub.get(h,0):.4f}" for h in HORIZONS))
print(f"  ★ Best 1-hour forecast: R²=0.6555")
