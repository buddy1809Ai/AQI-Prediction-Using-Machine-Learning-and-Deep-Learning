...
SEP = "=" * 72
print(SEP)
print("  TRACK A — XGBOOST — All Cities")
print(SEP)
...
for pq in parquets:
    ...
    safe_names = [c.replace("[","_").replace("]","_").replace("<","_") for c in feat_cols]
    import pandas as _pd_xgb
    X_tr_df = _pd_xgb.DataFrame(X_tr, columns=safe_names)
    X_te_df = _pd_xgb.DataFrame(X_te, columns=safe_names)

    t0  = time.time()
    mdl = XGBRegressor(n_estimators=300, max_depth=5, verbosity=0, random_state=SEED)
    mdl.fit(X_tr_df, y_tr)
    tr_t  = time.time() - t0
    yp    = mdl.predict(X_te_df)
    inf_t = time.time() - t0 - tr_t

    r2v   = r2_score(y_te, yp)
    maev  = mean_absolute_error(y_te, yp)
    rmsev = rmse(y_te, yp)
    results.append({"city": city, "model": "XGBoost",
...
track_a_xgb_df = pd.DataFrame(results)
if CKPT.exists():
    old = pd.read_csv(CKPT)
    track_a_xgb_df = pd.concat([old, track_a_xgb_df], ignore_index=True)
track_a_xgb_df.to_csv(CKPT, index=False)
print(f"\n  Saved: {CKPT}  ({len(track_a_xgb_df)} rows)")
